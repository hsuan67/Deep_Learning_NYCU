...
class ReplayMemory(object):
    ...
    def sample(self, batch_size, device):
        ...
        return (torch.tensor(np.array(list(x)), dtype = torch.float, device = device)
...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                state = torch.unsqueeze(torch.tensor(np.array(state.squeeze(-1),dtype=np.float32), device=self.device, dtype=torch.float32),0).to(self.device)
                return torch.argmax(self._behavior_net(state), 1).item()

    def append(self, state, action, reward, done, next_state):
        ## TODO ##
        """Push a transition into replay buffer"""
        self._memory.push(state, [action], [reward / 10], [int(done)], next_state)
    ...
    def _update_behavior_network(self, gamma):
        ...
        state, action, reward, done, next_state = self._memory.sample(self.batch_size, self.device)
        ## TODO ##
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            if total_steps < args.warmup:
                ...
                # select action)
                action = agent.select_action(np.array(state_list), epsilon, action_space)
                # decay epsilon
            ...
            ## TODO ##
            # store transition
            agent.append(state_list[0:4], action, reward, done, state_list[1:5])
...
def test(args, agent, writer):
    ...
    for i in range(args.test_episode):
        ...
        while not done:
            ...
            if args.render:
                env.render()
            action = agent.select_action(np.array(state_list), args.test_epsilon, action_space)
            next_state, reward, done, _ = env.step(action)
            state_list.append(next_state)
            state_list = state_list[1:5]
            e_reward += reward
...
def main():
    ...
    parser.add_argument('-tmp', '--test_model_path', default='ckpt/dqn_10600000.pt')
