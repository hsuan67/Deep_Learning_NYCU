...
- 反向傳播: 應用連鎖法則 (Chain Rule) 手動推導並實作梯度計算
- 參數調校: 分析不同學習率 (Learning Rate)、隱藏層單元數及 activation function 對 Loss 的影響
...
- 非線性必要性: 對於 XOR 類型的非線性問題，移除 activation function 會導致模型無法有效學習分類邊界，Loss 遠高於具activation function的版本
